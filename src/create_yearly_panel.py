...
# ---------------- CONFIG ---------------- #
MAX_YEAR = 2025  # Restrict panel to 2025
...
def create_yearly_panel(input_path, output_path):
    ...
    passed_years = df_starts[df_starts["event_types"] == "Passed/Approved"].groupby("Family ID")["year"].min()
    other_years = df_starts[df_starts["event_types"] != "Passed/Approved"].groupby("Family ID")["year"].min()
    ...
    # ---------------- Check missing start_year ---------------- #
    missing_start = policy_years[policy_years["start_year"].isna()]
    if not missing_start.empty:
        print("⚠ The following Family IDs have missing start_year and need manual attention:")
        print(missing_start.index.tolist())

    # Fill missing start/end years for processing
    policy_years["start_year"] = policy_years["start_year"].fillna(MAX_YEAR).astype(int)
    policy_years["end_year"] = policy_years["end_year"].fillna(MAX_YEAR).astype(int)

    # Cap end_year at MAX_YEAR
    policy_years["end_year"] = policy_years["end_year"].clip(upper=MAX_YEAR)

    # ---------------- Expand policies into yearly panel ---------------- #
    rows = []
    for fam_id, row in policy_years.iterrows():
        for year in range(row["start_year"], row["end_year"] + 1):
            rows.append({"Family ID": fam_id, "Year": year})
...
def main():
    ...
    parser.add_argument("--input", required=True, help="Path to input CSV (legis_df.csv)")
    parser.add_argument("--output", required=True, help="Path to output CSV (expanded panel)")
